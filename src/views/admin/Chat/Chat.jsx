...
=======
  ...
  const listenForTypingStatus = () => {
    console.log("all room information details ----> ", roomInfo);
    const typingRef = doc(
      db,
      "Chat",
      roomInfo?.roomId, // Room ID
    ...
    console.log("Listening for typing status on:", typingRef.path);
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          const receiverTypingStatus = typingData?.isTyping;

          console.log("Receiver typing status:", receiverTypingStatus);

          // Update receiver's typing status in the state
          setReceiverIsTyping(receiverTypingStatus || false);
        }
      },
      (error) => {
  ...
  useEffect(() => {
    if (roomInfo?.roomId && roomInfo?.receiverId) {
      // Start listening for typing status changes
      const unsubscribe = listenForTypingStatus();
  ...
  }, [roomInfo?.roomId, roomInfo?.receiverId]);

  console.log("listen status for typing---->", receiverIsTyping);
...
>>>>>>> Stashed changes
  return (
    <>
          ...
          >
            <div className="chatingHeader">
              ...
              <div className="cht-user">
                ...
                <p>{receiverIsTyping ? "Typing" : "Not Typing"}</p>
              </div>
            </div>
      ...
      />
      <MenuList>
        <MenuItem onClick={() => handleDeleteMessageFromMe(curr?.id)}>
          Delete for Me
        </MenuItem>
        <MenuItem onClick={() => handleDeleteMessage(curr?.id)}>
          Delete for Everyone
        </MenuItem>
      </MenuList>
    </Menu>
                      ...
                      <div className="cht_usr_txt">
                        ...
                        curr.messageType === "csv" ? (
                          ...
                          <div>
                            ...
                            {/* <p>{!receiverIsTyping?"Not Typing":"Typing"}</p> */}
                          </div>
                        )}
                      </div>
                    </div>
...
//   const listenToReceiverTypingStatus = (db, roomInfo, setIsTyping) => {
//     if (!roomInfo?.roomId || !roomInfo?.receiverId) {
//       console.error("Room ID or Receiver ID is missing");
//       return;
//     }
...
//       if (docSnapshot.exists()) {
//         const typingData = docSnapshot.data();
//         setIsTyping(typingData.isTyping); // Update the typing status in the state
//       } else {
//         setIsTyping(false); // Default to not typing if no data exists
//       }
//     });
...
//   useEffect(() => {
//     const unsubscribe = listenToReceiverTypingStatus(db, roomInfo, setIsTyping);
...
//   console.log("typing is running or not ---------> -------->", isTyping);
