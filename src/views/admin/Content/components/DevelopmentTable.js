...
export default function DevelopmentTable(props) {
  ...
  const [newUploadedContent, setNewUploadContent] = useState([]);
  const newGetLiveTaskUploadedContent = async (
    page,
    parametersName,
    parameters
  ) => {
    ...
    try {
      await Get(
        `admin/newGetalluploadedcontent?offset=${offset}&limit=${perPage}&${parametersName}=${parameters}`
      ).then((res) => {
        setNewUploadContent(res?.data?.response);
      });
    } catch (error) {
      setLoading(false);
    }
  ...
  useEffect(() => {
    newGetLiveTaskUploadedContent();
  }, []);

  // Update Uploaded Content -
  const [uploadedContentPayload, setUploadedContentPayload] = useState({
    index: 0,
    content_id: [],
    payload: {}
  });
  const updateUploadedContent = async (formData) => {
    const dataToSend = {
    ...
    setLoading(true);
    try {
      await Post("admin/updateUploadedContent", dataToSend)
        .then((res) => {
          newGetLiveTaskUploadedContent();
          setUploadedContentPayload({
            index: 0,
            content_id: [],
            payload: {}
          })
        ...
        <div className="">
          ...
          <TableContainer className="fix_ht_table">
            <Table mx="20px" variant="simple" className="common_table">
              ...
              <Tbody>
                {newUploadedContent?.map((curr, index) => {
                  const stripeFee = curr?.Vat?.[0]
                    ? curr.Vat[0].stripe_fee
                    : 0;
                  // const vatCharge = curr.Vat[0] ? curr.Vat[0].Vat : 0;
                  const imageCount = curr?.content?.filter(
                    (el) => el.type == "image"
                  )?.length;
                  const videoCount = curr?.content?.filter(
                    (el) => el.type == "video"
                  )?.length;
                  const interviewCount = curr?.content?.filter(
                  ...
                  return (
                    <Tr>
                      ...
                      <Td className="timedate_wrap">
                        <p className="timedate">
                          <img src={watch} className="icn_time" />
                          {moment(curr?.content[0]?.task_time).format(`hh:mm:A`)}
                        </p>
                        <p className="timedate">
                          <img src={calendar} className="icn_time" />
                          {moment(curr?.content[0]?.task_time).format(`DD MMM YYYY`)}
                        </p>
                      </Td>
                      <Td className="address_wrap">
                        {curr?.content[0]?.task_id?.location}
                      </Td>
                      <Td className="item_detail">
                        <img
                          src={curr?.content[0]?.brodcasted_by?.profile_image}
                          alt="Content thumbnail"
                        />
                        <Text className="nameimg naming_comn">
                          <span className="txt_mdm">
                            {curr?.content[0]?.brodcasted_by?.company_name}
                          </span>
                        </Text>
                      </Td>
                      <Td className="description_td">
                        <Text className="desc_ht">
                          {curr?.content[0]?.task_id?.task_description}
                        </Text>
                      </Td>
                      <Td className="text_center">
                        <div className="dir_col text_center">
                          {curr?.content[0]?.task_id &&
                            curr?.content[0]?.task_id?.need_photos === true ? (
                            <Tooltip label={"Photo"}>
                              <img
                                src={camera}
                                alt="Content thumbnail"
                          ...
                          {curr?.content[0]?.task_id &&
                            curr?.content[0]?.task_id?.need_interview === true ? (
                          ...
                          {curr?.content[0]?.task_id &&
                            curr?.content[0]?.task_id?.need_videos === true ? (
                            <Tooltip label={"Video"}>
                              <img
                                src={video}
                                alt="Content thumbnail"
                      ...
                      <Td className="text_center">
                        <Tooltip label={curr?.content[0]?.category_details?.name}>
                          {
                            <img
                              src={curr?.content[0]?.category_details?.icon}
                              className="icn m_auto"
                            />
                          }
                        </Tooltip>
                      ...
                      <Td className="text_center">
                        <div className="dir_col">
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.hopper_photo_price != null
                              ? curr?.content[0]?.task_id?.hopper_photo_price % 1 !== 0
                                ? curr?.content[0]?.task_id?.hopper_photo_price.toFixed(2)
                                : curr?.content[0]?.task_id?.hopper_photo_price
                              : "0"}
                          </p>
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.hopper_interview_price != null
                              ? curr?.content[0]?.task_id?.hopper_interview_price % 1 !==
                                0
                                ? curr?.content[0]?.task_id?.hopper_interview_price.toFixed(
                                  2
                                )
                                : curr?.content[0]?.task_id?.hopper_interview_price
                              : "0"}
                          </p>
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.hopper_videos_price != null
                              ? curr?.content[0]?.task_id?.hopper_videos_price % 1 !== 0
                                ? curr?.content[0]?.task_id?.hopper_videos_price.toFixed(
                                  2
                                )
                                : curr?.content[0]?.task_id?.hopper_videos_price
                              : "0"}
                          </p>
                        </div>
                      </Td>
                      <Td className="text_center">
                        <div className="dir_col">
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.photo_price != null
                              ? curr?.content[0]?.task_id?.photo_price % 1 !== 0
                                ? curr?.content[0]?.task_id?.photo_price.toFixed(2)
                                : curr?.content[0]?.task_id?.photo_price
                              : "0"}
                          </p>
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.interview_price != null
                              ? curr?.content[0]?.task_id?.interview_price % 1 !== 0
                                ? curr?.content[0]?.task_id?.interview_price.toFixed(2)
                                : curr?.content[0]?.task_id?.interview_price
                              : "0"}
                          </p>
                          <p>
                            £{" "}
                            {curr?.content[0]?.task_id?.videos_price != null
                              ? curr?.content[0]?.task_id?.videos_price % 1 !== 0
                                ? curr?.content[0]?.task_id?.videos_price.toFixed(2)
                                : curr?.content[0]?.task_id?.videos_price
                              : "0"}
                          </p>
                        </div>
                      </Td>
                      <Td className="item_detail">
                        <img
                          src={process.env.REACT_APP_HOPPER_AVATAR + curr?.content[0]?.avatar_details?.avatar}
                          alt="Content thumbnail"
                        />
                        <Text className="nameimg naming_comn">
                          <span className="txt_mdm">{`${curr?.content[0]?.uploaded_by?.first_name} ${curr?.content[0]?.uploaded_by?.last_name}`}</span>
                          <br />
                          <span>({curr?.content[0]?.uploaded_by?.user_name})</span>
                        </Text>
                      </Td>
                        ...
                        <div className="check_wrap">
                          <Checkbox
                            ...
                            onChange={(e) => {
                              setUploadedContentPayload({
                                index,
                                content_id: curr.content?.map((el) => el._id),
                                payload: {
                                  ...uploadedContentPayload.payload,
                                  firstLevelCheck: {
                                    ...uploadedContentPayload.payload.firstLevelCheck,
                            ...
                            onChange={(e) => {
                              setUploadedContentPayload({
                                ...
                                payload: {
                                  ...
                                  firstLevelCheck: {
                                    ...uploadedContentPayload.payload.firstLevelCheck,
                                    isDeepFakeCheck: e.target.checked
                                  }
                                }
                              })
                            }}
                      ...
                      {
                        console.log("uploadedContentPayload", uploadedContentPayload)
                      }
                        ...
                        <Checkbox
                          ...
                          onChange={(e) => {
                            setUploadedContentPayload({
                              ...
                              payload: {
                                ...uploadedContentPayload.payload,
                                checkAndApprove: e.target.checked
                              }
                            })
                          }}
                        />
                      ...
                      <Td className="big_select_wrap">
                        ...
                        >
                          ...
                          <option value="rejected">Rejected </option>
                          {(
                            uploadedContentPayload?.index === index &&
                            uploadedContentPayload?.payload?.checkAndApprove &&
                            uploadedContentPayload?.payload?.firstLevelCheck?.nudity &&
                            uploadedContentPayload?.payload?.firstLevelCheck?.isAdult &&
                            uploadedContentPayload?.payload?.firstLevelCheck?.isGDPR &&
                            uploadedContentPayload?.payload?.firstLevelCheck?.isDeepFakeCheck
                          )
                          ? (
                            <option value="published">Published</option>
                          ) : null}
                        ...
                        <Textarea
                          ...
                          onChange={(e) => {
                            setUploadedContentPayload({
                              ...
                              payload: {
                                ...uploadedContentPayload.payload,
                                remarksforliveUploaded: e.target.value
                              }
                            })
                          }}
                        />
                      ...
                      <Td>
                        <Button
                          ...
                          onClick={() => updateUploadedContent(uploadedContentPayload)}
                        >
                          Publish
                        </Button>
                        <Button
