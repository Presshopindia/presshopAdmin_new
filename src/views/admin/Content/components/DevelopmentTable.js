...
export default function DevelopmentTable(props) {
  ...
  const getBlockedContentList = async (
    ...
    try {
      await Get(`admin/getContentList?status=blocked&limit=${perPage}&offset=${offset}&${parametersName}=${parameters}&${parametersName1}=${parameters1}`).then((res) => {
  ...
  const CopyLInkOfproduct=async (index)=>{
   ...
   if (navigator.clipboard) {
    console.log("check12")
    navigator.clipboard.writeText(`https://new-presshop-mediahouse.web.app/Feeddetail/content/${content_id}`)
                ...
                contentList.map((value, index) => {
                  ...
                  return (
                    <Tr key={value._id}>
                      <Td  onClick={() =>
                          ...
                          {value?.content && value?.content?.length === 1 ? (
                            value?.content[0].media_type === "image" ? (
                              ...
                              <img
                              src={value?.content[0]?.thumbnail.startsWith("https") ? value?.content[0]?.thumbnail :`${process.env.REACT_APP_NEW_URL_THUMBNAIL}${value?.content[0].thumbnail}`}
                      ...
                      <Td>
                        {" "}
                        {formatAmountInMillion(value?.salePriceByAdmin ? value?.salePriceByAdmin :
